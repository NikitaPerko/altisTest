...
public class BoundsOctreeNode<T>
{
    ...
    public BoundsOctreeNode(Vector3 baseLengthVal, Vector3 minSizeVal, float loosenessVal, Vector3 centerVal)
    {
    ...
    public void DrawAllBounds(float depth = 0)
    {
        ...
        Bounds thisBounds = new Bounds(Center, adjSize);
    ...
    {
        Vector3 tintVal = BaseSize / 20;
        Gizmos.color = new Color(0, 1.0f - tintVal.x, tintVal.x, 0.25f);
    ...
    public BoundsOctreeNode<T> ShrinkIfPossible(Vector3 minLength)
    {
        if (BaseSize.x < 2 * minLength.x || BaseSize.y < 2 * minLength.y || BaseSize.z < 2 * minLength.z)
        {
            return this;
        }
        ...
        if (children == null)
        {
            ...
            SetValues(BaseSize / 2, minSize, looseness, childBounds[bestFit].center);
            return this;
        }
	...
	*/
    ...
    /// <param name="loosenessVal">Multiplier for baseLengthVal to get the actual size.</param>
    /// <param name="centerVal">Centre position of this node.</param>
    void SetValues(Vector3 baseLengthVal, Vector3 minSizeVal, float loosenessVal, Vector3 centerVal)
    {
        BaseSize = baseLengthVal;
        minSize = minSizeVal;
        looseness = loosenessVal;
        Center = centerVal;
        adjSize = looseness * baseLengthVal;

        bounds = new Bounds(Center, adjSize);

        Vector3 quarter = BaseSize / 4f;
        Vector3 childActualSize = (BaseSize / 2) * looseness;
        childBounds = new Bounds[8];
        childBounds[0] = new Bounds(Center + new Vector3(-quarter.x, quarter.y, -quarter.z), childActualSize);
        childBounds[1] = new Bounds(Center + new Vector3(quarter.x, quarter.y, -quarter.z), childActualSize);
        childBounds[2] = new Bounds(Center + new Vector3(-quarter.x, quarter.y, quarter.z), childActualSize);
        childBounds[3] = new Bounds(Center + new Vector3(quarter.x, quarter.y, quarter.z), childActualSize);
        childBounds[4] = new Bounds(Center + new Vector3(-quarter.x, -quarter.y, -quarter.z), childActualSize);
        childBounds[5] = new Bounds(Center + new Vector3(quarter.x, -quarter.y, -quarter.z), childActualSize);
        childBounds[6] = new Bounds(Center + new Vector3(-quarter.x, -quarter.y, quarter.z), childActualSize);
        childBounds[7] = new Bounds(Center + new Vector3(quarter.x, -quarter.y, quarter.z), childActualSize);
    }
    ...
    void SubAdd(T obj, Bounds objBounds)
    {
        ...
        if (!HasChildren)
        {
            ...
            if (objects.Count < NUM_OBJECTS_ALLOWED ||
                BaseSize.x < 2 * minSize.x ||
                BaseSize.y < 2 * minSize.y ||
                BaseSize.z < 2 * minSize.z)
            {
