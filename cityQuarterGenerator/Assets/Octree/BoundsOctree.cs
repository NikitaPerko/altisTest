...
public class BoundsOctree<T>
{
    ...
    readonly Vector3 initialSize;

    // Minimum side length that a node can be - essentially an alternative to having a max depth
    readonly Vector3 minSize;
    ...
    public BoundsOctree(Vector3 initialWorldSize, Vector3 initialWorldPos, Vector3 minNodeSize, float loosenessVal)
    {
        if (minNodeSize.x > initialWorldSize.x || minNodeSize.y > initialWorldSize.y || minNodeSize.z > initialWorldSize.z)
        {
